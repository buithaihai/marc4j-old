...
// AnselInputStreamReader published in his gedcom parser.
/**
...
public class AnselToUnicode {
	...
	while(true) {
		...
		if (i > 0 && j >= 0xE0 && j <= 0xFF) {
		    int u = convert2(j*256 + i);
		    if (u > 0)
			sb.append((char)u);
		    else
			sb.append((char)convert1(i));
		}
	    }
	}
    ...
    private static int convert1(int ansel) {
	switch(ansel) {
        ...
        default: return ansel;   // if no match return character
	} //end switch
    }

    private static int convert2(int ansel) {
	switch(ansel) {
