 ...
 * normalization. A stripped down version of 2.6 originating from the 
 * <a href="http://www.cafeconleche.org/XOM/">XOM</a> project is included in 
 * this distribution.
 * </p>
 * <p>
 ...
 * </p>
 * <pre>   writer.setConverter(new AnselToUnicode());</pre>
 * <p>
 * In addition you can perform Unicode normalization. This is for example 
 * not done by the MARC-8 to UCS/Unicode converter. 
 * With Unicode normalization text is transformed into the canonical composed form. 
 * For example &quot;a´bc&quot; is normalized to &quot;ábc&quot;.
 * To perform normalization set Unicode normalization to true:
 * </p>
 * <pre>   writer.setUnicodeNormalization(true);</pre>
 * <p>
 * Please note that it's not garanteed to work if you try to convert normalized 
 * Unicode back to MARC-8 encoding using {@link org.marc4j.converter.impl.UnicodeToAnsel}.
 * </p>
 * <p>
 * This class provides very basic formatting options. 
 * For more advanced options create an instance of this class with a
...
public class MarcXmlWriter implements MarcWriter {
   ...
   * @throws SAXException
   */
  public MarcXmlWriter(OutputStream out) throws SAXException {
    this(out, false);
  }
   ...
   * @throws SAXException
   */
  public MarcXmlWriter(OutputStream out, boolean indent) throws SAXException {
    if (out == null) {
  ...
  /**
   * Constructs an instance with the specified output stream, character encoding
   * and indentation.
   * 
  ...
  public MarcXmlWriter(OutputStream out, String encoding, boolean indent) {
    ...
    try {
      ...
      throw new MarcException("SAX error occured while writing start document",
          e);
    }
  }
   ...
   * 
   * @return boolean - true if this writer performs Unicode normalization, false
   *         otherwise.
   */
  ...
  private void writeStartDocument() throws SAXException {
    ...
    atts.addAttribute(Constants.MARCXML_NS_URI, "xmlns", "xmlns", "CDATA",
        Constants.MARCXML_NS_URI);
    handler
        .startElement(Constants.MARCXML_NS_URI, COLLECTION, COLLECTION, atts);
  }
  ...
  private void toXml(Record record) throws SAXException {
    ...
    while (i.hasNext()) {
      ...
      handler.characters(temp, 0, temp.length);
      handler
          .endElement(Constants.MARCXML_NS_URI, CONTROL_FIELD, CONTROL_FIELD);
    }
    ...
    while (i.hasNext()) {
      ...
      handler.startElement(Constants.MARCXML_NS_URI, DATA_FIELD, DATA_FIELD,
          atts);
      Iterator j = field.getSubfields().iterator();
      while (j.hasNext()) {
        Subfield subfield = (Subfield) j.next();
        atts = new AttributesImpl();
        atts.addAttribute("", "code", "code", "CDATA", String.valueOf(subfield
            .getCode()));
        ...
        handler
            .startElement(Constants.MARCXML_NS_URI, SUBFIELD, SUBFIELD, atts);
